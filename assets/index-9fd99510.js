import{s as h,h as g,N as C,a as B,b as E,c as q,d as T}from"./backdrop-heart-3e4a595e.js";const y=new C,M=document.querySelector(".my_title_category");function A(t){const o=document.querySelector(".list-category-books");t.forEach(e=>{const s=`
      <li class="list__category">${e.list_name}</li>
    `;o.insertAdjacentHTML("beforeend",s)})}function H(t){const o=document.querySelector(".books-container");let e=0;t.forEach(s=>{s.books.length!==e&&(e=s.books.length,M.insertAdjacentHTML("beforeend",`<h1 class="my_title">
    ...
    `;o.insertAdjacentHTML("beforeend",i)})}async function I(){h();try{const t=await y.fetchCategoryBooks();A(t)}catch(t){console.warn(t)}finally{g()}}async function p(){h();try{const t=await y.fetchTopFiveBooks();H(t)}catch(t){console.warn(t)}finally{g()}}I();p();const a=document.querySelector("#book-modal"),N=document.querySelector(".books-container");document.querySelector(".category-books-list");const j=document.querySelector(".close-button"),x=document.querySelector(".modal__add-book-btn"),m=document.querySelector(".modal__remove-notification"),u=document.querySelector(".modal__add-notification"),k=document.querySelector(".modal__notification");let b=document.querySelector(".modal__content"),r=null,n=[];x.addEventListener("click",P);j.addEventListener("click",d);N.addEventListener("click",U);function v(){!a.classList.contains("is-hidden")||(u.classList.remove("hidden"),m.classList.add("hidden"),k.classList.add("hidden"))}function O(){n.includes(r)?(k.classList.remove("hidden"),m.classList.remove("hidden"),u.classList.add("hidden")):(k.classList.add("hidden"),m.classList.add("hidden"),u.classList.remove("hidden"))}function z(t){return fetch(`https://books-backend.p.goit.global/books/${t}`).then(o=>o.json())}function F(t){const o=`
  ...
  `;b.innerHTML="",b.innerHTML=o}function R(t){z(t).then(o=>{F(o)})}function _(){n=JSON.parse(localStorage.getItem("shopping-list"))||[]}function D(){localStorage.setItem("shopping-list",JSON.stringify(n))}function J(){localStorage.setItem("shopping-list",JSON.stringify(n))}function d(t){t.code==="Escape"&&(a.removeEventListener("keydown",d),a.classList.add("is-hidden")),t.currentTarget===t.target&&a.classList.add("is-hidden"),t.target.classList.contains("popup-fade")&&a.classList.add("is-hidden"),t.target.classList.contains("modal__close-img")&&a.classList.add("is-hidden"),G(),v()}function U(t){if(!t.target.closest(".section-books-item"))return;const o=t.target,s=o.closest("[data-id]").dataset.id;if(window.addEventListener("keydown",d),a.addEventListener("click",d),a.addEventListener("keydown",d),o.classList.contains("books-btn"))return;r=s,R(s),a.classList.remove("is-hidden"),O(),Array.from(a.classList).includes("is-hidden")||Y()}function P(t){if(k.classList.toggle("hidden"),m.classList.toggle("hidden"),u.classList.toggle("hidden"),_(),u.classList.contains("hidden")&&(n.includes(r)||(n.push(r),D())),m.classList.contains("hidden")){const o=n.indexOf(r);o!==-1&&(n.splice(o,1),J())}}v();_();const L=document.documentElement;function Y(){L.style.overflow="hidden"}function G(){L.style.overflow="auto"}function K(){const t=document.querySelector("#scroll-top");t.addEventListener("click",e),window.addEventListener("scroll",o);function o(){const s=window.scrollY,i=700;t.classList.toggle("js-scroll-up_hidden",s<i)}function e(){window.scrollTo({top:0,behavior:"smooth"})}window.addEventListener("load",()=>{window.scrollTo(0,0)})}K();const f=document.querySelector(".books-container"),w=document.querySelector(".my_title_category");async function $(t){h();const o="https://books-backend.p.goit.global/books/category";try{const e=await T.get(o,{params:{category:t}});if(e.status!==200)throw new Error(`Request failed with status ${e.status}`);return V(e.data),e.data}catch(e){throw new Error(`Request failed: ${e.message}`)}finally{g()}}const Q=document.querySelector(".list-category-books");function V(t){let o="";t.forEach(e=>{e.list_name!==o&&(o=e.list_name,w.insertAdjacentHTML("beforeend",`<h1 class="my_title">${e.list_name}</h1>`));const s=`
    ...
    `;i.insertAdjacentHTML("beforeend",S)}),o.parentElement.insertBefore(i,o.nextElementSibling)}function tt(t){return fetch(`https://books-backend.p.goit.global/books/category?category=${t}`).then(o=>o.json()).catch(o=>{throw console.error("Error:",o),o})}const ot=document.querySelector(".all-category");ot.addEventListener("click",p);Q.addEventListener("click",W);f.addEventListener("click",X);const et=document.querySelectorAll(".books-btn"),st=document.querySelector(".books-container");et.forEach(t=>{t.addEventListener("click",nt)});function it(t){const o=t.map(e=>`<li>
        <div class="card-book">
          <div data-id=${e._id}>
              <img
              src="${e.book_image}"
    ...
    `).join("");st.innerHTML=o}async function at(t){const o=await $(t);it(o)}function nt(t){const o=t.target.dataset.id;at(o)}
